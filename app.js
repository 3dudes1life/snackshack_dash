...
const SnackCloud = (() => {
  ...
  const apiKey = () => String(localStorage.getItem("snackosSyncPassword") || window.SNACKSHACK_SYNC_API_KEY || "");
  ...
  async function request(path, options={}){
    ...
    if(!res.ok){ const text=await res.text(); const err=new Error(text || `Cloud error ${res.status}`); err.status=res.status; throw err; }
  ...
  async function hydrate(){
    ...
    try{
      ...
      setStatus("synced");
      return true;
    }catch(err){ console.warn("SnackCloud hydrate failed",err); setStatus("offline",err?.status===401?"Sync password needed · open Admin → Cloud Sync":"Using saved device data · cloud retry queued"); return false; }
  }
...
function renderAdmin(){const p=v300Prefs(),wrap=document.getElementById("workspaceToggles");if(!wrap)return;wrap.innerHTML=V300_SECTIONS.map(([id,label,desc])=>`<label class="switch-row"><input type="checkbox" data-workspace="${id}" ${(p.hidden||[]).includes(id)?"":"checked"}><span><b>${label}</b><small>${desc}</small></span></label>`).join("");document.getElementById("densitySetting").value=p.density;document.getElementById("focusModeSetting").checked=!!p.focus;const pw=document.getElementById("syncPasswordSetting");if(pw&&!pw.value)pw.value=savedSyncPassword();const rem=document.getElementById("rememberSyncPassword");if(rem)rem.checked=!!savedSyncPassword();applyV300Prefs();renderDiagnostics()}
function savedSyncPassword(){return localStorage.getItem("snackosSyncPassword")||""}
function saveSyncConnection(){
  const input=document.getElementById("syncPasswordSetting"), remember=document.getElementById("rememberSyncPassword");
  const value=String(input?.value||"").trim();
  if(value && remember?.checked)localStorage.setItem("snackosSyncPassword",value);
  else if(!remember?.checked)localStorage.removeItem("snackosSyncPassword");
  window.SNACKSHACK_SYNC_API_KEY=value;
  SnackCloud.setStatus("saving","Testing shared brain…");
  SnackCloud.refresh().then(()=>{renderDiagnostics();showToast("Cloud connection saved on this device")});
}
async function testSyncConnection(){
  const input=document.getElementById("syncPasswordSetting");
  window.SNACKSHACK_SYNC_API_KEY=String(input?.value||savedSyncPassword()||"").trim();
  SnackCloud.setStatus("saving","Testing shared brain…");
